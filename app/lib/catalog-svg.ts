  ...
  if (rotationalVectorKinds.has(element.kind)) {
    ...
    switch (element.kind) {
      ...
      case "smooth-incline": case "rough-incline":
        ...
        break;
      case "rigid-joint": body = `<circle cx="0" cy="0" r="7" fill="#18202b"/><line x1="${-w / 2}" y1="0" x2="${w / 2}" y2="0" ${common}/><line x1="0" y1="${-h / 2}" x2="0" y2="${h / 2}" ${common}/>`; break;
      case "distributed-load": body = `<line x1="${-w / 2}" y1="${-h / 2}" x2="${w / 2}" y2="${-h / 2}" ${common}/><line x1="${-w / 2}" y1="${h / 2}" x2="${w / 2}" y2="${h / 2}" ${common}/>${makeSvgText(0, -h / 2 - 14, label || "w", fontSize, true, rot)}`; break;
      case "triangular-load": body = `<line x1="${-w / 2}" y1="${h / 2}" x2="${w / 2}" y2="${-h / 2}" ${common}/><line x1="${-w / 2}" y1="${h / 2}" x2="${w / 2}" y2="${h / 2}" ${common}/>${makeSvgText(w / 4, -h / 2 - 10, label || "q", fontSize, true, rot)}`; break;
      case "bending-moment": body = `<path d="M ${-w * 0.3},${-h * 0.2} A ${w * 0.35},${h * 0.35} 0 1 1 ${w * 0.2},${h * 0.25}" fill="none" ${common}/>${makeSvgText(0, -h * 0.45, label || "M", fontSize, true, rot)}`; break;
      case "shear-diagram": body = `<path d="M${-w / 2},0 L${-w / 2},${-h / 2} L0,${-h / 2} L0,${h / 2} L${w / 2},${h / 2} L${w / 2},0 Z" fill="#3178d4" fill-opacity="0.15" stroke="#18202b" stroke-width="${lineWidth}"/><line x1="${-w / 2}" y1="0" x2="${w / 2}" y2="0" ${common}/>${makeSvgText(0, -h / 2 - 12, label || "SFD", fontSize, true, rot)}`; break;
      case "moment-diagram": body = `<path d="M${-w / 2},0 Q0,${h / 2} ${w / 2},0 Z" fill="#3178d4" fill-opacity="0.15" stroke="#18202b" stroke-width="${lineWidth}"/><line x1="${-w / 2}" y1="0" x2="${w / 2}" y2="0" ${common}/>${makeSvgText(0, h / 2 + 16, label || "BMD", fontSize, true, rot)}`; break;
